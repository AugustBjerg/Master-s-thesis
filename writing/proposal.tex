...
The aim is thus \textit{not} to evaluate the viability or impact of new technologies, such as green fuels, although these are recognized as necessary components of the decarbonization of the shipping sector.
...
The expected application is a decision-support workflow that converts high-frequency operational data into estimates of performance degradation that can be used to trigger maintenance actions, such as hull or propeller cleaning, when deterioration patterns indicate increasing inefficiency. The same outputs can be used to prioritize monitoring investments by identifying which operational and environmental variables contribute most to predicted inefficiency, and by deemphasising variables with limited marginal value. Model selection can be guided by an explicit accuracy versus complexity trade-off, which is relevant if inference is intended to run with constrained computational resources in near-real-time settings.

A second expected application is to translate operational inefficiency into implications for fuel use and emissions by comparing observed performance to a benchmark representing an operationally optimal state under comparable conditions. This supports an evidence-based discussion of operational measures, such as monitoring and maintenance planning, as complements to longer-horizon technology transitions that are outside the scope of the study. The analysis can also support internal performance reporting by separating variation that is plausibly linked to controllable factors from variation that is more consistent with exogenous operating conditions, subject to the available measurements and corrections.

Methodologically, the study is expected to contribute a replicable framework for constructing a data-driven operational baseline and attributing deviations using model-specific interpretation tools, designed for real operational datasets rather than idealized trials. Interpretation will remain conditional on modelling choices and data quality, and results should be treated as case-specific because the dataset relates to one anonymized vessel with an unknown route mix and operating context. Consequently, conclusions are expected to be strongest for within-vessel inference and weakest for claims about generality across fleets, unless future work extends the framework to additional vessels with comparable sensor coverage.
